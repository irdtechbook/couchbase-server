...
Queryサービス内部の処理、及び他のサービスとの連携は、以下のように行われます(可能な場合、処理は並行で実行されます)。
...
DCPストリーム上のデータミューテーション（更新）情報が、インデックスのターゲットを含む場合には、プロジェクターとルーターは、そのデータをスーパーバイザーに送信して、スーパーバイザーがインデックスを更新します。
...
（後に詳しく触れますが）全てのノードに存在するクラスターマネージャーの中から、マスターサービスとしてのクラスターマネージャーが選出されます。
...
クラスターマネージャーの中心となるプロセスはns-serverであり、以下の機能を持ちます。

 * @<strong>{マスターサービス} フェイルオーバー、リバランス、バケットの追加と削除など、クラスター全体に影響を与える運用を管理します。常に、クラスター上のノードの1つだけがマスターサービスの役割を担当します。マスターサービスは、ノード間でのネゴシエーションの上、決定されます。選出されたノードが使用できなくなった場合は、別のノードが引き継ぎます。マスターサービスは、オーケストレーターと呼ばれることもあります。

 * @<strong>{REST API管理} REST APIによる管理機能を提供します。これは、Webコンソールによって提供される機能とコマンドラインインターフェイスの両方の基礎になっています。

 * @<strong>{認証(Authentication)} ロールベースのアクセス制御でノードのリソースを保護します。各ロールはさまざまなシステム権限に関連付けられています。資格情報（ユーザー名とパスワード）に基づいて、ロールが割り当てられます。
...
マスターサービスは、クラスターのメンバー構成に責任を持ちます。クラスターのトポロジーが変更されると、既存のワークロードの処理を継続しながら、クラスターの再構成が行われます。

Dataノードが追加された際には、以下の一連の操作が実行されます。
...
Dataノードを削除するプロセスは、追加するプロセスと似ています。
維持されるノードで新たにvBucketsが作成され、削除されるノードのvBucketsからデータが移動されます。削除対象ノードから、vBucketがなくなった後に、そのノードはクラスターから削除されます。
...
Couchbase Serverバケットは、物理的に1024個のマスターvBucketファイルと、レプリカvBucketファイルからなります。
マスターサービスは可用性とリバランスのパフォーマンスを最大化するために、これらのvBucketsの配置をコントロールします。
