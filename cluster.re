...
クラスターから、機能しているノードを削除するには、まずノードを削除対象としてマークします。
...
ノード障害発生時に、クラスターを縮退稼働させるために、フェイルオーバーを手動で実行するか、あるいは、自動的にフェイルオーバーがトリガーされるようにします。
...
この時、ノード障害が発生し、ソフトウェアが自動的にノードをフェイルオーバーした場合、残りのノードでは負荷が増大し、正常に処理を続けることはできないでしょう。
...
もちろん、ノード障害発生時にも十分な余剰のキャパシティを確保し、縮退運転ができるようにしておくことが、望ましいのは言うまでもありません。
...
組織において、しかるべき意思決定者による判断を伴わずに、システムによって自動フェイルオーバーを実行することを、許可していないことも珍しくありません。
...
あるいは、システム利用状況監視情報との併用により、クラスターを縮退させても、残りのノードで集約したトラフィックを処理できると判断できた場合にのみ、フェイルオーバーを実行することも考えられます。
...
環境に合わせて、適切な待機時間を設定することは、サーバーやネットワークの一時的な問題により、機能しているノードをフェイルオーバーしてしまうことを防ぐために重要です。
...
グレースフルフェイルオーバーは、応答しているノードをクラスターから削除する際に用いられます。その場合、Couchbase Serverには、クラスターの削除という手段も存在します。それぞれの長所と短所を含め、グレースフルフェイルオーバーについて理解するために、ドキュメント@<fn>{couchbase-graceful-failover}を参照することができます。
