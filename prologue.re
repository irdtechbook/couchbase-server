...
//footnote[n1ql-explanation][上に紹介した論文には、「syntactic sugar over SQL++」という表現が見られます。]
...
「それで、どこが違うかが分からないという話に戻ると、確かに本番環境の構築となると神経を使うところはあると思いますが、Couchbaseの場合、開発用に、1ノードで構成されたクラスターを使っている時と、本番環境で、複数ノードのクラスターを構築した時とで、論理的な構成が変わらないことが、開発される立場から見られてもそうであるように、構築する立場の人にとっても、お言葉を借りるなら、シンプルさを提供してくれます」
