...
Couchbase Serverをインストールした際に同時にインストールされるコマンドは、下記のパス以下(ほとんどは@<tt>{bin}直下、一部@<tt>{tools}や、@<tt>{install}ディレクトリの下)に存在します。
...
入力に用いるファイルのフォーマットとしてCSVとJSONをサポートしています。

ここでは、基本的な機能を紹介し、最後に、CSVファイルからデータをインポートするケースについて、利用例を示して解説します。

詳細についてはドキュメント@<fn>{cbimport}を参照してください。
...
詳細な解説や、有効な接続文字列の実例について、ドキュメント@<fn>{cbimport-host-formats}を参照してください。
...
各行に、1つのJSONオブジェクトが記載されているファイルを入力とします。
...
@<tt>{-g}(@<tt>{--generate-key})オプションとして、データに含まれる@<tt>{user_id}が(@<tt>{%}で修飾して)指定されている他、@<tt>{--scope-collection-exp}オプションで、スコープ名に、@<tt>{region}が指定されているのが分かります。
...
@<tt>{cbexport}を用いて、Couchbase Serverから、データをファイルにエクスポートすることができます。
出力されるファイルのフォーマットはJSONです。

ここでは、基本的な機能を紹介します。詳細についてはドキュメント@<fn>{cbexport-json}を参照してください。
 ...
 * @<strong>{LINES} 各行が、1つのJSONオブジェクト
 * @<strong>{LIST} JSONオブジェクトを要素として持つ、1つの配列
...
ここで紹介しきれない、使用可能なオプションや、全てのオプションの詳細については、ドキュメント@<fn>{cbbackupmgr-doc}を参照してください。
...
@<tt>{cbbackupmgr}の使用を開始するにあたって、まずバックアップを保存するディレクトリを決定する必要があります。このディレクトリは、@<strong>{バックアップアーカイブ}と呼ばれます。

バックアップアーカイブには、@<strong>{バックアップリポジトリ}が含まれます。
バックアップリポジトリは、@<tt>{config}サブコマンドを使用して作成します。

このチュートリアルでは、@<tt>{/data/backup}をバックアップアーカイブとして使用します。
バックアップアーカイブには、空のディレクトリを選択します。
@<tt>{cbbackupmgr}コマンド初回実行時に、バックアップアーカイブとして構成されます。

バックアップリポジトリを構成する最も単純な方法は、クラスター全体をバックアップ対象とすることです。
...
バックアップリポジトリを作成する際に、バックアップの対象を選択することができます。
例えば、@<tt>{travel-sample}バケット内のインデックス定義のみをバックアップするとします。
...
バックアップリポジトリを作成したので、バックアップアーカイブの情報を確認します。
...
@<tt>{info}サブコマンドは、バックアップアーカイブ内のすべてのバックアップリポジトリとバックアップの情報を表示します。
...
バックアップの作成を開始します。バックアップリポジトリには、バックアップの作成方法に関する情報が含まれているため、バックアップ作成時には、バックアップアーカイブの場所と、バックアップリポジトリ(の名前)、そしてバックアップするターゲットクラスターの情報(認証情報を含む)を指定します。
...
「single」バックアップリポジトリは、@<tt>{travel-sample}バケットからインデックス定義のみをバックアップするように構成されているため、@<tt>{beer-sample}バケットの進行状況バーは表示されていません。
...
いくつかのバックアップが作成されたので、@<tt>{info}サブコマンドの出力内容が変わっています。「cluster」バックアップリポジトリには、バックアップが作成された時間に対応する名前のバックアップが1つ含まれていることがわかります。
そのバックアップには2つのバケットが含まれており、バケット内のミューテーション、トゥームストーン、インデックスなどの数を確認できます。
「single」バックアップリポジトリには1つのバックアップが含まれていますが、このバックアップには@<tt>{travel-sample}バケットのみが含まれ、データ項目は0個で、10個のインデックスがあることがわかります。
...
２回目以降のバックアップでは、バックアップリポジトリに既に存在しているデータは、バックアップの対象とはなりません。

増分バックアップの確認のため、一部のデータを変更した後、「cluster」バックアップリポジトリで@<tt>{backup}サブコマンドを再度実行してみます。
...
この実行例では、バックアップ実行前に2つのアイテムを更新して、2つのアイテムを新規作成しています。
バックアップされたのはそれらのデータのみであることが分かります。

@<tt>{info}サブコマンドを使用して、バックアップアーカイブの情報を確認します。
新しくバックアップが追加されており、そのバックアップの@<tt>{Items}の数は4で、そのすべてが@<tt>{Mutations}であることがわかります。これは、2つの変更と2つの追加に対応しています。
...
ここでは、「cluster」バックアップリポジトリの１回目のバックアップから復元します。
...
ここでは、バックアップを1つだけ復元するため、@<tt>{--start}と@<tt>{--end}の両方に同じものを指定しています。

また、@<tt>{--force-updates}フラグが利用されています。これにより、競合解決プロセスがスキップされ、クラスター上の値が新しく更新されている場合でも、バックアップされたデータで強制的に上書きします。
ここで、２回目のバックアップ前に更新した2つの値を確認すると、最初のバックアップを作成したときの値に戻っていることがわかります。
...
@<tt>{merge}サブコマンドを使用して2つ以上のバックアップをマージすることができます。

「cluster」バックアップリポジトリには2つのバックアップがあるので、これらのバックアップをマージしてみます。
...
マージ前の2番目のバックアップには、2つの新しいアイテムが含まれていました。
そのため、最初のバックアップのデータ項目数(31591個)から2つ増えています。
２番目のバックアップに含まれていた残りの2つのアイテムは、新規アイテムではなく更新であるため、マージ中に重複が取り除かれ、マージ後のバックアップのアイテムの個数に影響を与えていません。
...
バックアップリポジトリが不要になった場合は、@<tt>{remove}サブコマンドを使用してバックアップリポジトリを削除できます。
...
ここで@<tt>{list}サブコマンドを実行すると、「cluster」バックアップリポジトリが存在していないことを確認できます。
...
また、以下の方法で、Couchbase Serverがインストールされていない環境に、C SDKをインストールし、これらのユーティリティを用いて、リモートのクラスターに対して、操作を行うことができます。
...
追加のクライアントオプションを、キーと値のペアとして、Dスイッチ（@<tt>{-D})と共に指定することができます。下記に記述例を示します。
