 ...
 * @<strong>{マネージドキャッシュ}は、設定されたクォータに従って、バケットに割り当てられたメモリーです。アイテムが書き込まれると、アイテムはまずキャッシュに入り、その後、1つ以上の他のノードに複製するために、複製キューに配置されます。さらに（揮発性のEphemeralバケットではなく、Couchbaseバケットの場合）ディスクキューを介して、ディスクに書き込まれます。また、クラスター全体のさまざまなノード上のバケットアイテムの場所が記録されている、パーティションハッシュテーブルも、このマネージドキャッシュに含まれます。
