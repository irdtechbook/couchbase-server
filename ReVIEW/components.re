 ...
 * @<strong>{マネージドキャッシュ}は、設定されたクォータに従って、バケットに割り当てられたメモリです。アイテムが書き込まれると、アイテムはまずキャッシュに入り、その後、1つ以上の他のノードに複製するために、複製キューに配置されます。さらに（揮発性のEphemeralバケットではなく、Couchbaseバケットの場合）ディスクキューを介して、ディスクに書き込まれます。また、クラスター全体のさまざまなノード上のバケットアイテムの場所が記録されている、パーティションハッシュテーブルも、このマネージドキャッシュに含まれます。
 * @<strong>{チェックポイントマネージャー}は、アイテムの変更を追跡します。チェックポイントには、まだレプリケーションキューとディスクキューに配置されていないメモリ内の変更が記録されます。
 * @<strong>{アイテムページャー}は、最近使用されていないアイテムを、必要に応じてメモリから取り除き、スペースを解放します。
 * @<strong>{フラッシャー}は、バケット内のすべてのアイテムを一括削除することができます。バケットのデフォルト設定では、フラッシュ処理は無効になっています。
 * @<strong>{有効期限(Expiry)ページャー}は、期限切れのアイテムをスキャンし、メモリとディスクから消去します。有効期限ページャーは、デフォルトで60分ごとに実行されます。
...
さらに、決定されたパスを使用してクエリ実行プランを生成し、計画を一連の操作（オペレータ）にまとめます。
最後に、@<strong>{実行エンジン}がオペレータを受け取り、実行します。
...
====[column]エディションによる差異
コミュニティエディションではクエリの並列実行におけるリソース利用は、最大４コアに制限されているのに対して、エンタープライズエディションではこの制限がありません。

このことは、コミュニティエディションでは、ノード上のDataサービスを無効にすることができないため、QueryサービスとDataサービスは、常に同じハードウェアリソースを共有している、という事実と考え合わせて理解することができます。
...
インデックス作成時には、まずQueryサービスから、スーパーバイザーに対して、インデックス作成リクエストが送られます。
その後、スーパーバイザーは、インデックス定義をプロジェクターとルーターに渡します。

=== プロジェクターとルーター

プロジェクターとルーターはDataノード上で実行されるプロセスです。
 ...
 * @<strong>{マスターサービス} フェイルオーバー、リバランス、バケットの追加と削除など、クラスター全体に影響を与える操作を管理します。常に、クラスター上のノードの1つだけがマスターサービスの役割を担当します。マスターサービスは、ノード間でのネゴシエーションの上、決定されます。選出されたノードが使用できなくなった場合は、別のノードが引き継ぎます。マスターサービスは、オーケストレーターと呼ばれることもあります。
...
保守の対象には、クラスターマネージャーのもう1つのプロセスであるns-serverを含みます。
...
vBucketのコピーが完了すると、古いvBucketから新しいvBucketへのスイッチオーバーが発生し、新しいvBucketがアクティブになると、マスターサービスは、新しいvBucketマップをすべてのノードとクライアントに通知します。
