...
本章から、Couchbase Mobileの構成要素の一つであるSyng Gatewayを扱います。
...
レプリケーション(Replication@<fn>{wiki-replication})とは、字義通りには複製(Replica)を作ることと言えますが、コンピューティングの分野でデータレプリケーションといった場合には、一般に複数の環境に同じデータを保存することを指します。
その具体的な内容は多岐に渡ります。

Couchbase Mobileについて「レプリケーション」という用語が用いられる場合、それは複数のCouchbaseデータベース間の自動データ同期を指しています。@<fn>{memo-replication-meaning}

//footnote[memo-replication-meaning][Couchbase Mobileの文脈で、「レプリケーション」という用語が用いられる時には、Couchbase LiteとCouchbase ServerとのSync Gatewayを介した同期を指すことが大部分です。ただし、後述のSync Gateway間レプリケーションにおいては、Sync Gatewayを介したCouchbase Serverクラスター間のデータ同期を指すなど、必ずしもそれだけの意味に止まらないため、ここでは「Couchbaseデータベース間」という表現が選ばれています。]
...
Sync GatewayとCouchbase Liteとの同期(Sync with Couchbase Lite@<fn>{sync-using-app})を実行するにあたって、データの同期方向に関して、以下の３通りから一つを選択します。
...
例えば、アプリケーションのマスターデータのように、ユーザーによって変更されないデータについては、プルレプリケーションを使うことが考えられます。
また、単にアプリケーションからデータの収集を行うケース、例えばエッジ端末上のセンサーデータを収集するIoTアプリケーションや、端末上でデータ登録を行うアプリケーションでは、プッシュレプリケーションを使うことができます。
...
データが複数のデータベースにコピーされて管理されている状況では、あるクライアントの変更が、別のクライアントに対して反映される前に、そのクライアントが同じデータへの変更を行なうことが起こりえます。
このような状況では、競合(データ更新の不整合)が発生します。

こうした状況に対応するために、Couchbase Mobileでは、ドキュメントが作成、更新、または削除されるたびに新たに@<em>{リビジョン}が作成されます。
「リビジョン」という用語について、GitやSubversionのような、分散バージョン管理システムを利用したことのある人には特に説明を要しないと思われますが、「リビジョン」は「バージョン(番号)」のようなものと考えることができます。
...
一方で、リビジョンは、Couchbase Mobileを理解する上で、依然として重要な概念だといえます。
...
Sync Gatewayにおいて、ドキュメントへのアクセスは、@<em>{ユーザー}、@<em>{ロール}、@<em>{チャネル}という、3つのエンティティによって管理されます。
...
チャネルエンティティは、名前とドキュメントのリストで構成されます。
...
ユーザーエンティティは名前、パスワード、ロールのリスト、およびチャネルのリストで構成されます。
...
Sync Gatewayにおけるドキュメントの配布やユーザーアクセスを構成する方法には以下があります。
...
例えば、以下の図に示すように、ユーザーやロールへのチャネルの追加は、両方の方法で構成可能です。
...
例えば、Sync Gateway間レプリケーションはWebSocketに基づいており、Couchbase Liteとのレプリケーションに使用されるものと共通のプロトコルが用いられます。
...
例えば、AWS Wavelengthのようなサービスを活用し、キャリアネットワークにCouchbase Serverを配置して低遅延のアクセスを提供する際に、このような構成を取ることが考えられます。
