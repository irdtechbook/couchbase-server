...
Sync Gatewayで認証するための、もう1つの方法は、@<em>{セッション認証}です。
...
エンタープライズエディションの@<tt>{ReplicatorConfiguration}クラスには、@<tt>{setAcceptOnlySelfSignedServerCertificate}メソッドが存在し、自己署名された証明書のみを受け入れるようにレプリケーターを設定することができますが、このメソッドは、コミュニティエディションの@<tt>{ReplicatorConfiguration}クラスでは、利用できません。
...
証明書ピン留め(Certificate Pinning)とは、サーバの公開鍵を、アプリケーションに事前に配置する(ピン留めする)ことを指します。
これによりアプリケーションは、この公開鍵によって信頼されたサーバとのみ通信を行います。
...
中間者(MITM)攻撃の検出に関連する技術として相互TLS/mTLS(mutual TLS)があります。
mTLSでは、証明書を使用した通常のTLSサーバー認証に加えて、クライアントがTLSセッションをネゴシエートするときに、X.509証明書を提示し、対応する秘密鍵を所有していることをサーバーに対して証明します。
...
これは例えば、優先度の高いドキュメントを最初にプッシュしたり、ドラフト状態のドキュメントをスキップしたりするために用いることができます。
...
プルレプリケーションフィルターはダウンロードされるドキュメントに対して適用されます。
...
レプリケーターのアクティビティレベルには、以下の種類があります。
...
これによって、例えば、ユーザーに対してレプリケーションの進行状況を通知することができます。
...
以下は、アクティビティレベルと組み合わせて、状態をログ出力する例です。
