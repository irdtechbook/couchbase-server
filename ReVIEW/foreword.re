...
Couchbaseという名称で名指される技術には、大きくいって、次の２つがあります。

 * Couchbase Server
 * Couchbase Mobile

Couchbase Serverは、分散JSONドキュメントデータベース（distributed JSON Document database）です(ここでの英語による引用は、Open Source Projects from Couchbase@<fn>{7b7c6748332542cc032aa96a4fa60999}より)。Couchbase Mobileは、Couchbase LiteとSync Gatewayという２つの技術を包含する呼称です。Couchbase Liteは、モバイル端末および組み込みデバイス用のネイティブドキュメントデータベース(native document database for mobile and embedded devices)です。Sync Gatewayは、Couchbase MobileのデータをCouchbase Serverと同期するために使うことができるコンポーネント（components to sync to Couchbase Server）です。

====[column]Couchbase ServerとCouchbase Lite
Couchbase Liteは、組み込み用ネイティブデータベースとして、プラットフォーム毎に開発されています。
そこから明らかな通り、Couchbase ServerとCouchbase Liteとは、それぞれ異なる技術を基盤としており、「ドキュメント指向データベース」であることは共通しているものの、Couchbase Serverに関する本書の記述を、Couchbase Liteへ援用することはできません。 
...
Couchbase Serverは、エンタープライズエディションとコミュニティエディションの、２つの形態でバイナリが提供されており、その基盤としてオープンソースプロジェクト@<fn>{https://github.com/couchbase}が存在しています。
...
本書の記述は、基本的にコミュニティエディションに基づいていますが、エンタープライズエディション固有の機能についても、エディションによる差異を示しながら紹介しています。
...
インメモリデータベース(In-memory database@<fn>{f84c5e736c6060f55bd27cf70f1ea253})は、メモリ内でデータを管理することによって、高速な応答性能を実現するデータベースです。
一般に、データベースが、データ永続化装置(ハードディスク等)を用いた永続性を保証しているのに対して、インメモリデータベースを利用する際には、永続化の要件は必要に応じ外部で実現されます。

=== リレーショナルデータベース

リレーショナルデータベース(Relational database@<fn>{86b6b5174c671988f4e5dba65702a9c0})は、関係データベースとも呼ばれ、関係モデル（リレーショナルデータモデル）に基づいて、データを扱います。
...
データベースにおけるクエリ(Query@<fn>{5dbbc36d7860bdcf33f45d112820924c})は、データベース管理システムへの問合せ一般の表現として普通名詞化されています。
