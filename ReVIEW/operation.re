...
ここでは、バックアップはまだ作成されていないので、バックアップリポジトリーの情報のみが確認できます。


=== バックアップの作成


バックアップの作成を開始します。バックアップリポジトリーには、バックアップの作成方法に関する情報が含まれているため、バックアップ作成時には、バックアップアーカイブの場所と、バックアップリポジトリー(の名前)、そしてバックアップするターゲットクラスターの情報(認証情報を含む)を指定します。
...
「single」バックアップリポジトリーでも、バックアップを実行して、2つのバックアップ実行の違いを確認してみましょう。
...
いくつかのバックアップが作成されたので、@<tt>{info}サブコマンドの出力内容が変わっています。「cluster」バックアップリポジトリーには、バックアップが作成された時間に対応する名前のバックアップが1つ含まれていることがわかります。
そのバックアップには2つのバケットが含まれており、バケット内のミューテーション、トゥームストーン、インデックスなどの数を確認できます。
「single」バックアップリポジトリーには1つのバックアップが含まれていますが、このバックアップには@<tt>{travel-sample}バケットのみが含まれ、データ項目は0個で、10個のインデックスがあることがわかります。
...
「cluster」バックアップリポジトリーには2つのバックアップがあるので、これらのバックアップをマージしてみます。
