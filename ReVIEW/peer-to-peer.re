...
P2P通信の基本的な概念としては、あくまでコンピューター端末の役割について規定しているものであり、たとえばSkypeのようなサービスでP2Pアーキテクチャーが採用される場合、あるいはWinnyのようなファイル共有ソフトでP2Pネットワークが用いられる場合には、インターネット環境の利用を前提としていますが、モバイル/IoTに関連してP2Pが語られる際には、端末に備わっているWi-FiやBluetooth機能を利用した、オフライン環境での端末同士の直接的な通信というコンテクストで用いられるケースがみられます。
そのようなアプリケーションのひとつに、FireChat@<fn>{wiki-FireChat}があります。
