 ...
 * サポートされる言語（例えば英語）を持つ複数のプロパティに対する検索（通常のクエリでも、複数の条件を組み合わせることはできますが、シンプルな実装になる可能性があります）
...
次の例では、「overview」プロパティにFTSインデックスを作成します。
...
@<tt>{FullTextIndexConfiguration}を作成する際、複数のプロパティを指定することもできます。

== FTSクエリ

インデックスを作成すると、インデックス付きのプロパティに対してFTSクエリを作成して実行できます。
...
以下の形式で検索条件として照合するパターンを指定することができます。
...
FTSインデックス作成時に、複数のプロパティを指定することができます。
通常、検索語は、インデックス定義に用いられた全てのプロパティと照合されます。プロパティ名と「:」の組み合わせに続いて検索語を指定することで、インデックス中の特定のプロパティに対する照合を表現することができます。

プロパティ名と「:」の間にはスペースを含みません。「:」と検索語の間にはスペースを入れます。

次の例は、「linux」という単語がドキュメントの@<tt>{title}プロパティに存在し、「problems」という単語がFTSインデックス作成時に指定されたいずれかのプロパティに含まれるドキュメントを検索します。
...
NEARクエリでは、2つの検索語の間にキーワード@<tt>{NEAR}を置くことで、これらの検索語間の近接度を指定します。
...
以下の例は、「linux」という単語を含み、かつ「couchbase database」または「sqlite library」というフレーズの少なくとも1つを含むドキュメントを検索します。
