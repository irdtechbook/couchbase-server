...
コンテナに含まれるコマンド(例えばここでは@<tt>{cbbackupmgr}コマンド)を実行する場合、以下のように行います。
...
=== クラスターセットアップウィザード
 ...
 インストール直後の画面上の選択肢、「Setup New Cluster」と「Join Existing Cluster」の２つから、「Setup New Cluster」を選択します。
...
Webコンソールのクエリワークベンチを用いて、インデックスを作成する手順は、以下の通りです。
...
とはいえ、クラスターは、複数ノードで構成されることが前提としてあり、１ノードでの構成は、その1つの形態に過ぎないのも確かです。
そこで、ここでは既存のクラスターに対して、ノードの追加あるいは削除を行い、クラスターの構成を変更する手順について、概要を記します。
...
既にそのクラスターにバケットが作成されている状況を考えると、追加したノードを含めてバケットを均等に分散する必要があることが分かります。
...
リバランスが完了してはじめて、追加されたノードは、実際にクラスターのメンバーになります。
...
既にそのクラスターにバケットが作成されている状況を考えると、単にノードをクラスターから取り外してしまうと、バケットが一部分失われることになり適切ではありません。
