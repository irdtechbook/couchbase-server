...
クエリ文字列の利用には、コンパイル時に検証されず、タイプセーフではない等の欠点があります。が、Flutterのようなサポートされていないプログラミング言語のために、Couchbase Lite用のパッケージをC言語で構築する際には、クラスを介して実現するよりも、クエリ文字列を用いる方が柔軟で実装が容易である、という利点があります。
