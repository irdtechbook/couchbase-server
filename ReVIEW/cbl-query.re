...
Couchbase Liteクエリには、以下のふたつタイプのAPIが存在します。
...
ふたつのAPIは、プログラミング方法は異なるものの、機能レベルでは基本的に同じ機能を提供しますが、わずかながら機能における差分が存在します。

クエリビルダーAPIは、SQL++/N1QLクエリAPIが備えている機能の全てを備えている訳ではありません。
一方で、Couchbase Lite 2.xまではクエリAPIは存在しておらず、クエリビルダーが唯一のAPIでした。
ここからもわかる通り、この機能の差は利用において本質的な問題とはなりえないものであり、クエリビルダーAPIに備わっていない機能を利用するために、SQL++/N1QLクエリAPIと併用する必要性は基本的に生じません。
たとえば、データ型変換関数はクエリAPIでのみ提供されていますが、クエリビルダーAPIを使う場合は、プログラミング言語の機能を利用することができます。
...
クエリ文字列の利用には、コンパイル時に検証されず、タイプセーフではない等の欠点があります。
一方、Flutterのようなサポートされていないプログラミング言語のために、Couchbase Lite用のパッケージをC言語で構築する際には、クラスを介して実現するよりも、クエリ文字列を用いる方が柔軟で実装が容易である、という利点があります。
...
以下、チュートリアルUsing Couchbase Lite with Recycler Views@<fn>{tutorials-university-lister-android}の内容をベースに、Androidアプリ開発におけるライブクエリの利用について、実際のコーディング内容を紹介します。
