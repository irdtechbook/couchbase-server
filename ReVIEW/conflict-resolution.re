...
一方、開発者は要件に応じて、独自の競合解決ロジックを実装することもできます。

この章では、最初に競合が発生するシナリオを整理します。
そして、それぞれのシナリオにおけるデフォルトの自動競合解決ロジックと、カスタム競合解決ロジックの実装方法およびその典型的な設計について解説します。
...
たとえば、クライアントが一定期間オフラインになった後に、再度オンラインに復帰した場合には、オフライン期間中にそのクライアントによって行われた変更は、その間に他のすべてのクライアントによって行われた変更と実質的に「同時」に行われたものとみなされます。

=== Couchbase Liteでの競合検出

Couchbase Liteでの競合検出には、@<em>{データベース更新時}と@<em>{プルレプリケーション時}のふたつがあります。

たとえば、ドキュメントをローカルデータベースから取得し、変更を加えた上で保存する前に、そのドキュメントが、プルレプリケーションによってリモートデータベースの変更を反映して更新されていた場合、ドキュメントをローカルデータベースに保存しようとしたタイミングで競合が検知されます。
...
これは、データベース内のドキュメントが削除されている場合であっても同様です。つまり、結果として、削除されたドキュメントは復活することになります。
...
ドキュメントID設計上、このような状況を避けることができない場合には、新規ドキュメントの作成が意図せずに既存ドキュメントの更新となってしまわないように、カスタム競合解決を用いて、ドキュメントIDを変更して保存するといった対応を行う必要が生じます。
...
その方法のひとつは、ドキュメントの保存を実行する際に競合が発生していることを検知できるようにすることです。
...
この問題への典型的な解決策は、再試行のためのループを導入することです。
...
ひとつの方法として、競合が発生しているドキュメントをマージすることが考えられます。
...
マージをどのように処理するかはアプリケーションの仕様によって異なりますが、この例では、保存されているドキュメントに新しいドキュメントのプロパティーを上書きしています。
...
これは、構成プロパティー@<tt>{custom_conflict_resolver}に、JavaScript関数を定義することによって行われます。
