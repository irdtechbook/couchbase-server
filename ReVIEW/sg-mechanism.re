...
エンタープライズエディションでは、@<tt>{database.cache.rev_cache}プロパティを使用して、リビジョンキャッシュのサイズを設定できます。

この設定を調整することにより、Sync Gatewayのメモリ消費量を制御できます。
これは、メモリが限られている環境を利用する場合、ドキュメント作成・更新が頻繁に発生する場合に重要になります。
