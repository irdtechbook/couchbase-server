...
@<tt>{cblite}は、Couchbase LabsのGitHubリポジトリ The Couchbase Mobile Tool Repo@<fn>{couchbase-mobile-tools}から入手することができます。
...
@<tt>{check}や@<tt>{compact}のように、開発やトラブルシューティングに利用できるものであっても、追加のフラグやパラメーターが存在せず特に解説を要しないものは扱っていません。
上掲のサブコマンド表も参考ください。
...
フラグ	効果
-----------------
@<tt>{--key KEY}	指定したキー(ドキュメントプロパティ)とその値のみを表示します。複数個同時に指定できます。
...
なお、@<tt>{cp}サブコマンドでは、Couchbase Mobileの重要な機能のひとつである二つのデータベース間のデータ同期(レプリケーション)操作についてもカバーされていますが、ここではレプリケーションに関連する記述については割愛しています。必要に応じ、ドキュメント@<fn>{cblite-documentation}を参照ください。
 ...
 * @<tt>{.json}で終わるJSONファイル。1行に1つのJSONドキュメントを含むファイルで有る必要があります。
 * @<tt>{/}で終わるディレクトリ。ディレクトリにはJSONファイル(ドキュメントごとに1ファイル)が含まれている必要があります。
...
@<tt>{--jsonid} @<b>{property}	ドキュメントIDに使用するJSONプロパティを指定します。詳細、下記参照ください。
 ...
 * @<b>{source}がJSONの場合、ドキュメントIDとして使用するプロパティ名を指定します。省略した場合、ドキュメントIDにはUUIDが用いられます。
 * @<b>{destination}がJSONの場合、ドキュメントIDを追加するためのJSONのプロパティ名を指定します。このフラグを省略すると、プロパティ名はデフォルトで @<tt>{_id}になります。
...
ディレクトリパス(@<b>{directory})を指定すると、そのディレクトリ内のすべての @<tt>{.cbllog}ファイルが読み取られます。
...
このプラグインのソースコードは、Couchbase LabsのGitHubリポジトリvscode-cblite@<fn>{vscode-cblite}で公開されています。
最新のアップデートや利用方法について、このリポジトリのREADMEを参照することができます。

=== データベース内容の確認

プラグインの利用を開始するには、はじめにCouchbase LiteデータベースをVSCodeのサイドパネルのエクスプローラー画面で開きます。
ここで、選択するCouchbase Liteデータベースは「〜.cblite2」という名称のフォルダになります(そのフォルダの中にあるファイルを選択するのではないことにご注意ください)。
...
すると、エディター画面にJSONドキュメントが１行で表示されます。エディター画面上で右クリックメニューから[Format Document]を選択することで、プロパティ毎に改行されたフォーマットでドキュメントの内容を表示することが可能です。
