...
機械学習、深層学習の成果が実用化されるとき、多くの場合、そのサービスは、クラウドとのやりとりで実現されてきたといっていいでしょう。
そのようなサービスの例として、たとえば、スマートスピーカーや、Web上やデスクトップアプリケーションで提供される自動翻訳機能があります。
このようなサービスは、データがクラウドに送られることで成立しています。
一方、iPhoneのFace IDでの顔認識モデルは、端末上で実現されています。
このような端末上でのAI機能は、エッジAIといわれています。

エッジAIの文脈において重要な要素として、セキュリティーやプライバシーの観点があります。

機械学習にはモデルの学習と推論の二面がありますが、学習済みのモデルを使った「推論」機能を、エッジ環境で提供することにより、データをサーバーへ送信する必要がなくなります。
また、局所的な学習結果を連合(Federate)してひとつのモデルを構築する、Federated Learning@<fn>{Federated_learning}のような、モデルの「学習」のためのテクノロジーも登場しています。


このようなニーズや背景を踏まえ、TensorFlow Lite@<fn>{tensorflow_lite}やPyTorch Mobile@<fn>{pytorch-mobile}のような、エッジAIを実現するためのオープンソースのソフトウェアライブラリーが存在しています。
また、iOSのCoreML@<fn>{coreml}や、AndroidのNeural Networks API(NNAPI)@<fn>{android_nnapi}のような、プラットフォームサポートも登場しています。これらは、プラットフォーム固有の開発言語における機械学習の利用手段(API)を提供するだけでなく、テンソル演算専用のチップセットの活用のように、ハードウェアレベルでの最適化についても実現できるようになっています。
...
Couchbase Liteの予測クエリは、エッジでの学習済みモデルを使った推論のためのものであり、このようなハードウェアおよびソフトウェアライブラリーの隆盛と本質的な関係があります。
...
予測クエリは、CoreMLやTensorFlow Lite、PyTorch Mobileのようなエッジデバイス上での機械学習モデルによる推論機能を提供するライブラリーと組み合わせて利用します。
...
ここで、アタッチするモデルは特定の仕様に縛られるものではなく、実際には機械学習モデルを内部で利用している通常の関数を登録します。
