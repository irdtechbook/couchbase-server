...
そのため、モバイルアプリケーション開発においては、多くの場合、アプリケーション内部のデータ管理と、サーバーとのデータ通信、というふたつの技術的要件が存在します。
...
「サーバーとのデータ通信」という観点において、ローカルデータベースとリモートデータベースとの間のデータ同期(レプリケーション)という手法は、選択肢のひとつということは言えますが、決して王道とはいえないでしょう。
...
たとえば、想像しやすいところでは、飛行機の搭乗員用のアプリケーションを考えてみることができます。@<fn>{customers-united-airlines}
このような設計を指して、@<strong>{オフラインファースト}という言葉が使われます。

一方で、昨今では「ネットワーク通信が行えない環境」と言われても、非常に特殊なケースという印象を持つ方も多いかもしれません。
その意味では、オフラインファーストアプリケーションは、@<strong>{ユーザーエクスペリエンス向上}の一形態と考えた方がわかりやすいかもしれません。
...
また現在では、ひとつのサービスが、モバイルアプリケーションとWebアプリケーションの両方の形態で提供されることも珍しくありません。
...
通信ロジックを個々のアプリケーションのために、新たに開発しなくても済むことが開発コストの削減につながることは見やすいところです。
...
エッジコンピューティングにおける重要な要素として、エッジデータセンターの活用があります。
...
エッジデバイスからデータ収集を行う際の構成要素として、オープンソーステクノロジーとしては、たとえば以下のようなものがあります。
...
以下は、上記エンティティーに対応するDAO(Data Access Object)定義です。
内部で、SQLクエリが用いられているのがわかります。
...
以上からわかる通り、Realmは、あらかじめデータモデルの定義が必要なオブジェクト指向データベースであって、MongoDBやCouchbase Serverのようなドキュメント指向データベースとは異なります。
...
Couchbase Liteでは、以下のようにドキュメントオブジェクト(@<tt>{MutableDocument})に対して、プロパティーとして値を設定します。
...
Couchbase Liteは、SQL準拠のクエリが利用できるため、たとえば以下のようなWHERE句を用いて、特定の種類のドキュメントを検索することができます。@<fn>{memo-cbs-sql}
...
データモデリングにおけるそれぞれの違いは、一次的にはそれらの特徴であって、優劣ではないということができます。
一方で各OSが提供する独自の技術は、その存在意義を示すために、SQLiteと比べて優位性を強調しています。

各OSが提供する独自の技術は、そのOS専用にアプリケーションを構築する場合には、固有の能力を発揮します。
一方で、同じ目的を持ったアプリケーションをそれぞれのOSのために作成する際の合理化のために、RealmやCouchbase Liteのような、中立のテクノロジーを活用することが考えられます。
...
それぞれについて注目すべき特徴や、あるいは、ここに挙げた以上の機能の存在、あるいは今後の機能拡張があるかもしれませんが、それらについては是非それぞれのデータベースに関する情報へ直接当たっていただきますようお願いします。
...
Firebaseの提供するローカルデータサポートは、一時的なオフラインのためのキャッシュとしてのものであるといえます。
