...
NoSQL登場以前には、データベースの分類といったとき、RDBとDWH（データウェアハウス）という区分が最も定番といえるものでした。そしてそれは、基盤系と情報系というシステムの分類に対応していました。また、OLTP（OnLine Transaction Processing）とOLAP（OnLine Analytical Processing）という用語を、ここで思い起こしてもいいでしょう。
...
高いトランザクションレベル（悲観的ロック）を取ることが絶対に重要で、システムに必要な要件であるユースケースがあります（その場合、メリット・デメリットを比較する必要さえないため、ここでは必ずしもそのことを前提としないケースについて検討します）。一方、前者の欠点はアプリケーション全体において、トランザクション/ロックの要件が必要である場合には、それ自体単体で見れば悲観的ロックと比較して軽量な処理であるはずの楽観的ロックの実装が問題となりうることを示しています。
...
このように、Couchbase Serverはデータのエンティティーに関する表現(コレクション)を持ち、テーブル(表)形式のデータを難なく扱えます。そのため、RDBのデータ(モデル)をCouchbase Serverに移行(応用)することは、造作なく行えます。
...
JSONのツリー構造は、より単純なデータ構造にも適用できるため、たとえばレガシーシステムをモダナイズする場合のように、データ構造の柔軟性が必ずしも必要ではないプロジェクトを含め、Couchbase Serverを活用することのできる場面は多岐に渡って存在します。
