 ...
 * @<tt>{doc}: ドキュメントのコンテンツを参照します。ユーザー定義のプロパティに加え、以下のプロパティを含みます。
 ** @<tt>{_id}: ドキュメントID
 ** @<tt>{_rev}: リビジョンID
 ** @<tt>{_deleted}: ドキュメント削除フラグ(ドキュメント削除の場合に含まれ、プロパティの値は@<tt>{true}です)
...
開発者は、Sync Gateway関数を定義する際に、要件に応じた検証ロジックを実装し、検証に適わない場合には例外により処理を中断することによって、ドキュメントの更新を制御することができます。
...
この場合、単に何も起こりません。
これにより@<tt>{channel(doc.channels)}のような使い方をする時に、@<tt>{doc.channels}が存在するかどうかを確認しなくても利用することが可能になっています。

以下の例は、ドキュメントの「published」プロパティが真の場合、ドキュメントの「channels」プロパティに定義されているチャネルに割り当てます。
...
指定されたチャネル(の少なくとも1つ)にアクセスできるユーザーによって行われていないドキュメントの更新を拒否します。
...
次の例は、ユーザーがドキュメントの@<tt>{channels}プロパティに定義されているチャネル(の少なくとも1つ)にアクセスできない場合、例外をスローします。
...
指定されたロール(の少なくとも1つ)を持っているユーザーによって行われていないドキュメントの更新を拒否します。
...
次の例は、ユーザーがこれらのロールの少なくとも1つを持っていない限り、例外をスローします。
...
例えば、以下のような用途に利用することが考えられます。

//blankline


 * 特定のユーザーや、実行コンテクスト以外でのドキュメントの変更を許可しないように、要件に応じて実装された検証プロセスに適っていない場合に、例外を発生させます。
 * ドキュメントに対して特定の構造を強制するために、必要な制約をチェックし、それらが満たされていない場合に例外を発生させます。例えば、必須プロパティや、充足していなければならないプロパティの組み合わせのチャック等が考えられます。
 * 無効な更新を許可しないために、更新前ドキュメント(@<tt>{oldDoc})と更新後ドキュメント(@<tt>{doc})とを比較し、不適切な変更が行われている場合に例外を発生させます。例としては、「ドキュメント作成者」のような、ドキュメントの新規作成以降は変更を許さないプロパティが考えられます。
...
上記の関数がコールされると、ドキュメントの更新は、HTTP 403 "Forbidden"エラーコードで拒否され、@<tt>{forbidden:}プロパティの値がHTTPステータスメッセージになります。
...
Sync関数のなかでドキュメントを検証するロジックを実装する際には、更新後のドキュメントを、つまり更新後ドキュメント情報を保持する@<tt>{doc}パラメータのすべてのプロパティを、基本的に「信頼できないもの」として扱うべきだといえます。
これは、関数は常に、その検証対象とするデータへの変更が破壊的なものでないこと、ないし悪意による改変が含まれていないことを前提とするべきではない、という考え方によります。

例えば、以下のコードは一見問題ないように見えます。
...
ドキュメント情報に基づくアクセス権限などの検証を実装する場合には、@<tt>{doc}ではなく、@<tt>{oldDoc}のプロパティを利用することが、往々にして適当だといえます。

一方で、以下のように、実行ユーザーが更新後ドキュメントのプロパティの値と一致することを検証するケースも考えられます。
...
削除されたドキュメントは、@<tt>{_deleted:true}プロパティを持つ単なるリビジョンです。
そして削除されたドキュメントには、@<tt>{_deleted:true}以外のプロパティは存在しません。
...
@<tt>{doc._deleted}プロパティをチェックして、削除時には不要な更新時の検証をスキップすることによって、削除時には存在しないプロパティへアクセスすることにより発生する意図しないエラーを回避します。。
...
要件定義着手にあたって、例えば以下のような観点が考えられます。
 ...
 * ドキュメントは、必ず次のプロパティを持つ： @<tt>{title}, @<tt>{creator}, @<tt>{channels}, @<tt>{writers}
 * @<tt>{editor}ロールを持つユーザーへのみドキュメントの編集(作成・変更・削除)を許可
 * ドキュメントの@<tt>{writers}プロパティに登録されているユーザーへのみドキュメントの変更(削除を含む)を許可
 * ドキュメントの@<tt>{creator}プロパティに登録されているユーザーによるドキュメント作成のみを許可(ドキュメント新規作成時に、実行ユーザーと@<tt>{creator}プロパティに登録されているユーザーが一致している)
 * ドキュメントの@<tt>{creator}プロパティが変更されない
...
Couchbase Serverは、ドキュメントに対して、拡張属性(Extended Attributes@<fn>{java-sdk-xattr})を設ける機能を提供しています。
開発者は、拡張属性(XATTR)を使って、ドキュメントにユーザー定義のメタデータを設定することができます。

拡張属性は、通常のアプリケーションロジックではなく(この場合、ドキュメントプロパティを利用するのが適当)、ライブラリーやフレームワークでの利用が想定されています。

エンタープライズエディションでは、Sync関数内でドキュメントの拡張属性にアクセスすることができます。
これによって、例えばアクセス制御の目的で利用される、Sync関数専用のデータ項目を、ドキュメントのプロパティを使用するよりも、安全かつ効率的に保持することができます。
