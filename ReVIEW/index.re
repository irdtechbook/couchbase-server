...
Couchbase Serverでは、RDBとは異なり、クエリの実行に際して、基本的に、対応するインデックスの存在が前提となります。
...
Couchbase Serverにおけるインデックスは、グローバルセカンダリインデックス（GSI）と呼ばれています。別に「プライマリインデックス」、「セカンダリインデックス」という表現も用いられますが、これらはいずれも、グローバルセカンダリインデックス（GSI）の中のカテゴリーとなります。（以下は、「プライマリインデックス」のメタデータの例です。@<tt>{"is_primary": true}かつ @<tt>{"using": "gsi"} であることが分かります）
...
====[column]エディションによる差異
メモリー最適化GSIは、コミュニティエディションでは利用することができません。
...
次の例に示すように、明示的に、名前を付けて定義する構文が基本となります。
...
Couchbase Serverのコア機能は、アプリケーションのバックエンドデータベースとして、設計・最適化されており、このインデックスの仕様は、一般的なアプリケーションでは、ユーザーにより恣意的に記述されるクエリ(アドホッククエリ)は発生しない(必要とされない)ことを反映しているといえるでしょう。柔軟性は性能とのトレードオフになり得ることを考えると、これは合理的な仕様だといえます。

一方、コア機能を補完する別のサービスが提供されており、例えば、データ分析者によるヒストリカルデータへのアドホックなクエリという要件については、(エンタープライズエディションで提供される)Analyticsサービスが対応します。また、自由な語句の入力による全文検索という要件については、(本書では扱わない)Searchサービスが提供されています。
...
スカラー値のみではなく、JSONの他のデータ形式（配列、オブジェクト）を値として持つフィールドを用いることも可能ですが、この場合、述語条件の右側の値は同様に構造化されたデータ形式(配列、オブジェクト)である必要があります。
...
次に、@<tt>{schedule}(フライトスケジュール)の@<tt>{flight}(フライト番号)と@<tt>{day}(運行曜日)の情報を使ってインデックスを作成します。
...
下記のDDLでは、「@<tt>{ARRAY ... END}」構文の内部で、「 @<tt>{WHEN v.day < 4}」という検索条件が用いられているのが分かります。
...
③ @<tt>{ANY-SATISFIES}句のもう１つの条件「@<tt>{v.day = 1}」は、インデックス定義の@<tt>{WHEN}句の条件「 @<tt>{v.day < 4} 」に包含されます。
...
エンタープライズエディションでは、1つのインデックス定義DDLを使って、下記のように複数のノード指定を行うことができます。
...
ここでは、限られた例を示すに留めますが、例えば、次のインデックス定義DDLはドキュメントキーでパーティション化されたインデックスを作成します。
...
これにより、Queryサービスのプロセスのメモリー領域がボトルネックとなる事を避けることができます。さらに、Couchbase Serverでは、ドキュメントキーによるデータ取得はそのドキュメントを管理しているノードから直接行われるため、1つの大きなネットワーク伝送を、多数の小さなネットワーク伝送に置き換えることが可能になります。
