...
結果セットとして、特定のプロパティのみを取得することも、ドキュメント全体を取得することもできます。
...
すべてのプロパティを取得するには、@<tt>{SelectResult.all()}メソッドを使います。
...
この場合の結果セットは、配列の要素オブジェクトごとにデータベース名を唯一のキーとして持ち、その値はドキュメントの各プロパティをキーと値のペアとする辞書型オブジェクトです。
...
以下は、すべてのプロパティを取得した場合の結果セットの利用例です。
@<tt>{Result}オブジェクトにはデータベース名をキーとした辞書型オブジェクトのエントリーがただ一つ存在します。
...
特定のプロパティを指定して取得するには、クエリのSELECTステートメントで、プロパティごとに1つずつ、カンマで区切られたアイテムのリストを用います。
...
この場合、結果セットの配列に含まれる要素オブジェクトは、指定したプロパティのキーと値のペアで構成されます。
...
以下は、特定のプロパティを取得した場合の結果セットの利用例です。
...
以下のコードは、「type」プロパティが「hotel」に等しいドキュメントを選択しています。
...
次の例では「name」プロパティの値が、「Foo」、「Bar」または「Baz」のいずれかに等しいドキュメントを検索します。
...
次の例では「first」、「last」または「username」のいずれかのプロパティの値が「Cameron」に等しいドキュメントを検索します。
...
以下の例では、@<tt>{"eng%e%"}という指定により、「name」プロパティの値に対してワイルドカードマッチを行います。
...
以下の例では、@<tt>{"eng____r"}という指定により、「name」プロパティの値に対して、キャラクターマッチを行います。「eng」で始まり、その後に正確に4つの任意の文字が続き、最後が「r」で終わる文字列に一致するドキュメントを検索しています。
...
以下の例では、「name」プロパティの値が、「eng」で始まり「r」で終わる任意の文字列に一致するデータを検索しています。
...
JSONデータは、プロパティの値としてnullを指定できます。
一方、存在していない値について、プロパティ自体を設けないこともできます。

NULLと未定義値を区別する必要はない場合があります。
その場合に、クエリを簡潔に表現することのできる@<tt>{isValued}と@<tt>{isNotValued}メソッドが用意されています。@<fn>{Expression-isValued}
これらは、プロパティ自体が定義されていないことと、プロパティの値がnullであることを区別しません。

次の例は、「email」プロパティが定義されており、かつnull以外の何らかの値を持っているドキュメントを検索しています。
...
WHERE句の中で、配列型のプロパティを持つドキュメントに対して、配列の要素を用いたフィルタリングを行うことが可能です。


指定された値が配列に存在するかどうかを確認するために@<tt>{ArrayFunction}クラスを利用できます。

次の例では、配列型の値を持つ「like」プロパティに「Graham Nash」と等しい値が含まれているドキュメントを検索します。
...
Couchbase LiteのN1QLは、配列型のプロパティに対する、より複雑な検索条件を利用する方法を提供しています。
...
それぞれ「type」プロパティとして「airline」と「route」を持ち、「route」タイプのドキュメントには、「airline」タイプのドキュメントのドキュメントIDが、プロパティ「airlineid」として格納されています。
...
次の例では、「name」プロパティの値の昇順で並べ替えられたドキュメントが返されます。
...
これは例えば、大量の結果を返すクエリでページネーションを処理するために利用することができます。
...
Couchbase Liteは、JSONドキュメントのプロパティとしてDateタイプ@<fn>{querybuilder-lbl-date-time}をサポートしています。Dateタイプは内部的に、ISO8601形式の日付(GMT/UTCタイムゾーン)を持っています。
