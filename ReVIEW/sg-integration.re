 ...
 * 各着信要求のURLを書き換えます。例えば、Sync GatewayのパブリックREST APIポートを外部公開用のポートにマップします。
...
Sync Gatewayとクライアント間の接続を保護するには、トランスポート層セキュリティ(TLS)を使用します。
...
メトリクスREST APIには、以下のような2つのエンドポイントがあり、JSONまたはPrometheus形式のいずれかのレポートフォーマットでデータを受け取ることができます。
