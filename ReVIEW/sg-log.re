...
@<tt>{logging.console.rotation}プロパティーを使用してログのローテーションを構成します。
...
ロギングは、ノードレベルのプロパティーとして、ブートストラップ構成にて管理されます。
...
ブートストラップ構成ファイルのプロパティーの詳細については、ドキュメント(Bootstrap Configuration@<fn>{configuration-schema-bootstrap})を参照ください。


=== 管理REST API

ロギング設定を、管理REST APIを使って動的に変更することも可能です。
ただし、管理REST APIによるロギングプロパティーへの変更は再起動後は引き継がれません。
