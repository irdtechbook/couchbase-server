...
ここでは、クライアント/アプリケーションからみたデータについて解説します。
Couchbase Serverでは、データは複数のノードに跨って管理されますが、データを利用する場合、表面的には、そのことを意識する必要はありません。
 ...
 * @<strong>{Couchbaseバケット}は、非同期でデータの永続化を行います(同期的な永続化を指示することも可能)。可能な限り多くのデータをメモリに保持しようとします。
 * @<strong>{Ephemeralバケット}は、データに対する永続化の要件のない場合に用いられ、データはメモリ上にのみ存在します。
...
標準SQLをJSONデータへのアクセスのために拡張したクエリ言語であるN1QLクエリでは、FROM句にコレクション名を用いますが、RDBからデータ構造を変更せずに、Couchbase Serverへそのまま移行した場合、論理的に同じクエリを用いることが可能です。
...
====[column]スコープとコレクションの用語法について
 ...
 ご存知のない方へ向けてお伝えすると、同じドキュメント指向データベースであるMongoDBでもドキュメントの格納単位として「コレクション」という用語が用いられています。
 Couchbase ServevrはMongoDBに対してその登場自体もドキュメントをグループ化するための機能の追加も後発ですが、いたずらに異なる用語を導入するよりも好ましいと言えるかもしれません。

「スコープ」という表現は、SQLにおけるコンテキストとの関係で理解することができます。SQLクエリは、特定のスキーマのコンテキストの元、実行されます。
この時、テーブル名が一意に定まるのは、そのコンテキストの「スコープ」においてです(「スコープ」と「コンテキスト」という用語は、プログラミング言語でも、テーブル名のかわりに変数に関して、同じ用法で用いられます)。
