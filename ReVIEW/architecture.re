...
分散アーキテクチャーでは、複数のサーバーを用いて、1つのプラットフォームを構成します。これを@<strong>{クラスター}と呼びます。また、クラスターを構成する各サーバーは@<strong>{ノード}と呼ばれます。
...
//footnote[performance-note][1ノード当たりの性能として、1秒間に数十万のリクエストを処理し、かつ、リクエストの99パーセントで1ミリ秒未満の応答時間が測定されることも珍しくありません。]
...
他の分散アーキテクチャーを持つソフトウェアでは、ノードは全て同一のリソースを持つハードウェアで構成されることが必要ないし前提とされているのが屡々見られるのに対して、Couchbase Serverは、リソース配分の柔軟性を特徴としており、各サービスに異なるノード数を割り当てられるだけでなく、各ノードでワークロード/サービスの特性に応じた異なるリソース(メモリー、CPU)を用いることができます。
