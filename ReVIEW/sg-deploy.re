...
Sync Gatewayを介したCouchbase LiteとCouchbase Serverとのデータ同期を行うに際して、Sync Gatewayエンティティーに由来する観点に加えて、Couchbase Serverの技術仕様に応じた考慮も必要になります。

また、Sync Gatewayのデプロイメント(Deployment@<fn>{sync-gateway-deployment})における、ハードウェアやOSに関する留意事項についても解説します。

== エンティティー設計

ユーザーやチャネルのような、Sync Gatewayで用いられるエンティティーの数は、プロジェクトの要件によって異なります。

エンティティー情報は、ディスクやメモリーのスペースを消費するため、要件に応じて、それらに関わるデフォルト値を変更したり、名前設計を考慮する必要が生じる場合があります。
...
ただし、Sync Gateway内部における情報の保存方法に関係して、実際上の上限が存在します。
...
ユーザーやドキュメントあたりのチャネル数が使用可能なスペースを超えることが見込まれる場合、次のことを検討します。
...
特に、コンシューマー向けモバイルアプリのように、サービスの利用規模拡大に合わせて、ユーザー数が増加する様なユースケースでは注意が必要になります。
...
Sync Gatewayには、ドキュメントのプロパティーに使われる予約済みのキーワードが存在します。
予約済みのキーワードは、@<tt>{_id} 、@<tt>{_rev}、@<tt>{_sequence}のようにアンダースコア(@<tt>{_})を接頭辞として持ちます。
先頭にアンダースコアが付いたユーザー定義のプロパティーがドキュメントのトップレベルに含まれる場合、Sync Gatewayによって同期を拒否されます。
その場合、次のようなエラーが発生します。
...
Couchbase Liteは、このようなプロパティー名の使用を明示的にエラーとはしません。
これは、ドキュメントをCouchbase Serverと同期せずにローカルのみで使用する場合には制約とならないためです。
Couchbase Serverとの同期を行う場合は、Couchbase Liteでアンダースコア文字から始まるプロパティー名を使用することを避ける必要があります。
...
Sync Gatewayのメタデータの内容をアプリケーションロジックのために、使用することはできません。
...
なお、コミュニティーエディションでは、これらのプロパティーが設定されていても無視され、デフォルト値が適用されます。
