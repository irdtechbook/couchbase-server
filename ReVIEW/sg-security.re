...
Sync Gatewayへのアクセスに関するセキュリティー(Secure Sync Gateway Access@<fn>{secure-sgw-access})機能について解説します。@<fn>{memo-what-about}

//footnote[memo-what-about][ここでは、Sync Gatewayで行うセキュリティー構成について記述しています。Sync Gatewayに接続するCouchbase Liteアプリケーションで行うセキュリティー構成については、後のCouchbase Liteレプリケーションの章を参照ください。]
...
Sync Gatewayの通信におけるセキュリティーを理解する際には、Sync Gatewayがサーバーの役割を果たす場合と、クライアントの役割を果たす場合とを区別することが重要になります。

たとえば、Sync GatewayにおけるTLS対応というとき、それは直接的にはまずサーバーとしてのセキュリティー機能として考えられますが、Sync GatewayがクライアントとしてCouchbase Serverへ接続する際の構成オプションとしての面も存在します。
...
認証プロバイダーから提供されているSDKを利用して、トークンを取得する責任はアプリケーションにあります。
...
TLS証明書認証を有効にするには、次のふたつの構成プロパティーを追加します。
...
これらのプロパティーが存在する場合、Sync Gatewayインスタンスは、TLSのみに応答します。
HTTP接続とHTTPS接続の両方をサポートしたい場合は、ふたつの別々のSync Gatewayインスタンスを実行する必要があります。
...
これらのファイルをSync Gatewayプロセスが読み取り可能なディレクトリーに配置します。
...
Couchbase ServerとSync Gatewayとの間でX.509証明書ベースの認証を使用するには、はじめにCouchbase Serverでルート証明書とサーバー証明書を作成します。
ルート証明書とサーバー証明書によってCouchbase Serverクラスターを保護した後で、クライアント証明書をルート証明書で署名します。
...
なお、Sync Gateway構成情報内に、Couchbase Serverのユーザー名/パスワードとX.509証明書に関するプロパティーの両方が指定されている場合、パスワードベースの認証が行われた上で、TLSハンドシェイクの際にクライアント証明書が用いられます。

X.509証明書ベースの認証は、以下のSync Gateway構成プロパティーに各ファイルへのパスを指定することによって有効になります。
...
構成プロパティーの詳細については、ドキュメント(Bootstrap Configuration@<fn>{configuration-schema-bootstrap})を参照ください。
...
適切と思われる構成を行ったにも関わらず接続が失敗する場合は、クラスター証明書エラー表(Cluster Certificate Errors@<fn>{handle-certificate-errors})を問題解決に役立てることができます。
...
一方、Sync GatewayではTLSの利用にエディションによる制約はなく、コミュニティーエディションでも用いることができます。
この不整合は将来何らかの形で解消されるかもしれませんが、現時点においてはコミュニティーエディションユーザーにとって混乱の原因となる可能性があるため、記します。
