...
はじめに、Couchbase Serverにおけるセキュリティー要素の概観を示します。@<fn>{security-overview}
...
認証には資格情報をサーバーに直接渡す方法と、資格情報が埋め込まれているクライアント証明書を使用する方法があります。

=== 認可 (Authorization)

Couchbase Serverは、ロールベースのアクセス制御(RBAC)によって、ユーザーを個別に割り当てられた役割に関連付けます。ロールは、特定のシステムリソースへの特定の操作を許可する権限のセットです。
...
コミュニティーエディションには、ユーザーをグループ化する機能はなく、ロールはユーザーのみに割り当てることができます。

コミュニティーエディションにおけるロールの位置づけは、限られたビルトインロールの提供に留まっており、本格的なロールベースのアクセス制御(RBAC)とは異なります。
...
Couchbase Serverのセキュリティーを高めるために、管理可能な領域について紹介します。@<fn>{security-management-overview}
...
暗号スイート、TLSレベル、およびコンソールアクセスを個別に管理することが可能です。
...
また、REST APIを使って設定を行うことも可能です。@<fn>{rest-set-password-policy}
...
外部システムで認証されるユーザーをクラスターに追加することができます。外部ユーザーは外部システムで認証されるため、クラスターでは認証情報(パスワード)を管理しません。
...
他のデータベースには、ユーザーの情報をデータベース(システムテーブル)で管理し、参照することのできるものもあります。Couchbase Serverはそれらとは異なり、ユーザー情報の管理・参照のために、システムテーブルは利用されていません。
