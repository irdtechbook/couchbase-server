...
単一障害点を避け、高可用性を確保するためには、少なくとも2つのSync Gatewayノードが必要になります。
...
このような一元化永続的モジュラー構成によって、例えば、ノード毎にそれぞれの構成ファイルを管理する場合と比べて、シンプルかつ俊敏な構成の更新が可能になっています。
 ...
 * @<em>{リモート管理}: Sync Gatewayノードのトポロジーに左右されず、容易にSync Gatewayを管理することができます。これは、エッジデータセンターにSync Gatewayがデプロイされている場合に特に有用です。また、構成情報の変更・確認においては、ユーザー認証とロールベースのアクセスコントロールが適用されるため、セキュリティを担保しながら容易な管理を実現することができます。

//blankline


Sync Gatewayの構成プロパティは、ノードに固有の@<em>{ノードレベルプロパティ}と、Sync Gatewayクラスター内で共有される@<em>{データベースレベルプロパティ}のふたつのレベルのいずれかに属します。

=== ノードレベルプロパティ

ノードレベルプロパティはノード固有であり、他のノードとは共有されません。
以下のようなプロパティが、ノードレベルプロパティに属します。

//blankline

 * @<em>{Couchbase Server接続設定}: Sync GatewayノードがCouchbase Serverへ接続するために必要な情報であり、構成プロパティの最小限セットです。変更には再起動が必要です。
 * @<em>{システムプロパティ}: たとえば、TLS認証ファイルのパス(@<tt>{api.tls.cert_path})や、サーバー固有のファイルデスクリプター最大値(@<tt>{max_file_descriptors})のようなシステム固有のプロパティです。変更には再起動が必要です。
 * @<em>{ロギングプロパティ}: ノードのログ出力に関するプロパティです。管理REST APIにより変更可能であり、その場合、再起動は不要ですが、REST APIによるロギングプロパティへの変更は、再起動後は引き継がれません。

=== データベースレベルプロパティ

データベースレベルプロパティは、クラスターを構成するSync Gatewayノードで共有されます。
これらのプロパティは、管理REST APIにより変更可能です。
以下のようなプロパティが、データベースレベルプロパティに属します。

//blankline

 * @<em>{DBプロパティ}: @<tt>{bucket}のようなデータベース構成や@<tt>{users}のようなデータベースへのアクセス制御ポリシーに関するプロパティです。
 * @<em>{レプリケーションプロパティ}: レプリケーションに関するプロパティです。
...
//footnote[memo-foreground][サーバーソフトウェアをこのようにフォアグラウンドプロセスで実行することに疑問を持たれる方がいるかもしれませんが、ここでは分かりやすさを優先しています。Sync Gatewayをサービスとしてバックグラウンドで実行する方法について、後の環境構築についての章で紹介しています。]
...
データベースの設定が含まれていないという警告が出力されていますが、管理REST APIによる構成を受け付けるために、Sync Gatewayが実行されているのが分かります。

====[column]構成グループ

Sync Gatewayエンタープライズエディションではクラスターのノードを、共通の構成を共有するグループに所属させることが可能です。
共通の構成を共有するノードのグループを@<em>{構成グループ(Config Group)}と呼びます。
グループIDプロパティ（@<tt>{bootstrap.group_id}）を使用して、Sync Gatewayノードをグループ化します。

1つのノードに行われた変更は、グループ内の他のノードに自動的に伝播されます。
...
本格的な複数台ノードからなるクラスター運用では、一元化永続的モジュラー構成が有益ですが、例えば開発用に環境を複製する場合など、再現性のある構成情報を簡単に共有するために静的な構成ファイル管理の利点を活用することができます。
...
Couchbase Mobileのドキュメントやサンプルでは、無名関数が用いられている場合と、関数に(@<tt>{sync}の様な)名前が定義されている場合の両方が見られます。

構成プロパティの値として定義された関数は、何らかの変数に代入された上で利用されている(つまり、特定の関数命名規則に依存しているのではない)と考えると、この記載方法の違いが本質的ではないことが理解できるでしょう。
...
アクセス制御のために、管理REST APIを利用して各種アクセス権限管理エンティティを構成する方法を解説します。

Couchbase Mobileにおけるレプリケーションの構成単位は、Couchbase Serverのバケットです。
このCouchbase Serverの「バケット」は、他のデータベースにおける「データベース」に類する位置づけと捉えることができ、Sync Gateway構成プロパティ上の命名としても「データベース(Database/DB)」という語が用いられています。アクセス制御の設定は、この「データベース」単位に行います。
...
構成プロパティの詳細については、ドキュメント(Configure Database@<fn>{lbl-configure-db})を参照ください。
...
@<tt>{admin_channels}プロパティの値に割り当てるチャネルを指定します。
...
ユーザー@<tt>{Edge1User}には、@<tt>{Edge1User}自身の@<tt>{admin_channels}プロパティを通じて @<tt>{Channel1}が割り当てられており、@<tt>{Edge1}ロールの@<tt>{admin_channels}プロパティを通じて@<tt>{Channel2}と@<tt>{Channel3}が割り当てられています。
ロールの@<tt>{admin_channels}の内容を確認しなくとも、ユーザーの@<tt>{all_channels}プロパティから、これらのチャネルの全てを確認することができます。
