...
デフォルトではSync Gatewayが起動すると、データベース構成プロパティーに従って、Sync Gatewayとデータベースとの接続がオンラインになります。
Sync Gatewayの起動時にデータベースをオフラインにしておきたい場合は、@<tt>{offline}構成プロパティーをデータベース構成プロパティーに追加します。
...
このようなオフライン/オンライン制御機能が提供されている理由のひとつとして、Sync Gatewayでは複数のデータベース用に構成することができるため、それぞれのデータベースの運用を個別に行う手段が提供されているといえます。

あるいは、Sync Gatewayがただひとつのデータベース用に構成されている場合に、クライアントに対してはオフラインにしておきながら、Sync Gatewayとデータベースの接続を維持しておく、Sync Gateway特有の理由として、再同期があります。
この再同期については、後に説明します。
...
複数のSync Gatewayノードからなるクラスター環境では、再同期の実行はひとつのノードでのみ行います。
