...
アプリケーションの要件や、システム環境に応じて、意思決定が必要な設定の紹介を通じて、Couchbase Serverに特有の概念や機能に関して理解を深めていただくことを目的としています。
...
Couchbase Serverのバケットには、メモリクォータが設定され、専用のメモリ領域が確保されます。
Couchbase Serverは、このメモリ領域を用いて、可能な限りすべてのデータをメモリ上に保持しようとします。

メモリ内のデータがバケットのメモリクォータの85％（ハイウォーターマークと呼ばれます）に達した場合、データがメモリから排出(削除)され始めます。この際、最近使用されていないデータから優先的に排出されます。
この排出プロセスは、メモリクォータの75％（ローウォーターマークと呼ばれます）を下回るまで続きます。
...
メモリからのデータ排出時のオプションとして、次のいずれかの方法を選択します。@<fn>{a-tale-of-two-ejection-methods-value-only-vs-full}

 * @<strong>{Value-only} 値のみが排出され、キーとメタデータはメモリに残ります。より多くのメモリが必要ですが、最高の性能が提供されます。
 * @<strong>{Full} （キー、メタデータ、値を含む）すべてが排出されます。メモリ要件を軽減します。



//footnote[a-tale-of-two-ejection-methods-value-only-vs-full][ブログ記事「A Tale of Two Ejection Methods: Value-only vs. Full」(https://blog.couchbase.com/a-tale-of-two-ejection-methods-value-only-vs-full/)にて、オプションの選択が、性能とメモリ消費に与える影響が詳細に解説されています。]
 ...
 * @<strong>{ドキュメント数の増加ペースの予測が難しい} キーやメタデータは、それ自体のサイズは決して大きくありませんが、ドキュメント数に応じて増えるため、全てがメモリに収まりきれなくなる事態が起こりえます。この場合、@<b>{Full}を選択することで、メモリ容量超過の問題を回避することができます。
...
ここでは、機能を紹介するためにダッシュボード画面を示しましたが、Couchbase Serverでは、ダッシュボードと同内容の監視項目をコマンドラインや、REST APIを用いて利用することができるため、これらを外部監視システムとの連携のために利用することが考えられます。
...
また、Couchbase Serverには、特定の監視項目について警告を発生させる機能があり、メタデータオーバーヘッドに関する警告を利用することができます。
...
この結果として生じる、物理データファイルと論理データとの(特にデータが削除された場合に顕著な)ギャップは、@<strong>{コンパクション}と呼ばれるプロセスを通じて、定期的に解消されます。
...
Couchbase Serverでは、コンパクションが実行される前に、ディスク領域がチェックされます。使用可能なディスクスペースが現在のデータファイルサイズの2倍未満の場合、コンパクション処理は実行されずにログに警告が出力されます。

=== オートコンパクション

オートコンパクション実行のトリガーとして、下記の２つがあります。

 * @<strong>{データファイルの断片化} データファイルが断片化されている割合をパーセンテージで設定します。例えば、10%に設定されている場合、データファイルが10%断片化された時点でオートコンパクションが実行されます。
...
1つのサーバーに複数のアドレスが割り当てられることは珍しいことではありませんが、分散アーキテクチャーを持つクラスターにおいては、モノリシックなアーキテクチャであれば考慮する必要がない面を、考慮する必要があります。

Couchbase Serverでは、クライアント/アプリケーションがクラスターに接続する際には、クラスターマップという要素が介在します。
初期接続は、特定のノード（複数指定可能）に対して行われますが、接続確立（ブートストラップ）時に、接続したノードからクラスターマップを受け取り、それ以降のリクエストは、対象となるノードに対して直接行われます。

そのため、ブートストラップのためにクライアント/アプリケーションが明示的に利用している特定のアクセス先へのリクエストだけを考えて、IPやドメイン名の解決が外部的に整合性が取れていれば良い訳ではなく、クラスターを構成する全てのノードに関する内部情報(クラスターマップ)が代替アドレスと整合している必要があります。

=== 外部ネットワークからの接続

外部ネットワークからクラスターへ接続するケースについて、下記に図示します。

まず注意しておくと、この図ではアプリケーションが外部ネットワークにあり、内部ネットワークにあるCouchbase Serverを利用していますが、これは必ずしも典型的なケースとは言えません。
一般的には、アプリケーションとCouchbase Serverは同じ内部ネットワークにあり、その場合、アプリケーションとCouchbase Serverとの接続には、プライベートIPが使われれば事足ります。この場合、代替アドレスを設定する必要はありません。
...
このケースでは、外部からの参照のために、ネットワークアドレス変換（NAT）機能で、各サーバーにパブリックIPを付与し、それらを代替アドレスに割り当てます。

プリンシパルアドレスには、プライベートIPアドレスが利用されます。ここではクライアントからの接続に焦点を当てていますが、プリンシパルアドレスは、クライアントからの接続のみではなく、クラスター内のノード間の接続にも利用されることは、理解しておく価値があります。
このことから、プリンシパルアドレスに、単に外部アドレスを割り当てることは適切ではない、ということがわかります。
...
ここでは、2つのサーバーがあり、それぞれプライマリアドレスおよびセカンダリアドレスを持っています。それぞれのアドレスを、プリンシパルアドレスと代替アドレスに割り当てています。そして、２つのアプリケーションは、それぞれ割り当てられたプリンシパルアドレス、または代替アドレスのいずれかを利用します。
