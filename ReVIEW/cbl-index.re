...
インデックス作成はオプションであり、インデックスが作成されていないプロパティを検索条件に利用することが可能です。
...
以下の例は、ドキュメントのプロパティ@<tt>{type}と@<tt>{name}に対して、新しいインデックスを作成します。
...
インデックスが作成されているプロパティがクエリの検索条件として利用されている場合、インデックスが利用されます。
ただし、クエリの内容によって、以下のような条件があります。

=== LIKE検索
LIKE検索では、次の条件が満たされている場合のみインデックスが使用されます。
...
インデックスの作成されているプロパティを検索条件に用いる際、そのプロパティに対して関数が適用されているとインデックスは利用されません。

例えば、以下のようなクエリでは、@<tt>{name}プロパティに対してインデックスが作成されている場合であっても、そのインデックスは利用されません。
