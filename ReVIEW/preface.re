...
なお、書名としては語句が重複するため避けましたが、「モバイル/エッジコンピューティング〜」とする方が実情を上手く表しているため、本文中ではこの表現を用いています。

最後に、「エッジ」という言葉について補足します。
IoT(Internet of Things)隆盛の中、「エッジ」は、IoTデバイスの同義としても使われていますが、「エッジコンピューティング」という時、それは「クラウドコンピューティング」への対比としての意味を持ちます。
...
その後、Couchbase Liteを使ってアプリケーションを開発するための情報を提供する幾つかの章が続きます。
...
Couchbase Mobileは、エンタープライズエディションとコミュニティエディションの、２つの形態でバイナリが提供されており、その基盤としてオープンソースプロジェクト@<fn>{couchbase-oss}が存在しています。
本書の記述は、基本的にコミュニティエディションに基づきます。
...
本書中に掲載しているサンプルコード等を以下のURLで公開しています。また、本書に関する情報共有のために必要に応じこのリポジトリを更新する予定です。
