...
「後は、iOSのCore Dataや、AndroidのRoom、それから、こうしたプラットフォーム固有の技術以外の選択肢として、Realmを取り上げようと思っています。
モバイル用データベースは他にもあると思うんですが、その点で知恵を貸していただければと思って、相談させていただきました」
...
「まだ調べきれてはいませんが、RealmがMongoDBと同期できるということまでは分かりました。MongoDBはNoSQLデータベースですよね。それで、他のNoSQLデータベースでも同じような機能を持ったものがあるかもしれないと思ったんです」
...
「そうなんですね。違う種類というのは分かりましたが、実際どういう違いなんでしょうか？」

「Javaの経験があるということだから、オブジェクト指向は分かりますよね?」
...
「この違いそれ自体は、優劣ではなく、性質の違いと言えるでしょう。RealmはそもそもMongoDBのフロントエンドとして開発されたわけではなく、単体の組み込みデータベースとして開発されたものなので、アプリケーション内で定義されたデータを保存するという観点からは、データモデルの定義を前提とするオブジェクト指向という性質は自然、という見方もできると思います」
...
「あくまで名前の由来という意味でね。NoSQLの特徴の一つである分散アーキテクチャーを持つサーバーソフトウェアと、組み込みデータベースという根本の違いがあるので、言葉通りに受け取るのは危険ですが」
...
「そうだね。エッジでのユースケースとして、例えばセンサーデータを収集するアプリケーションを考えてみると、センサーの扱うデータ項目が変更された場合にも、ドキュメント指向データベースであれば対応が容易という面があります」
...
「なるほど。押さえておくべきことが分かってきました。自分でも調べてみますが、何か気をつけておくべきことなどありますか?」

「クラウドやデータセンターとの同期やMBaaSまで視野に入れているのであれば、サーバーサイドのデータベースについてもそれなりに把握しておくことが重要になるかと思います。例えば、ドキュメント指向データベースについては、MongoDB、Couchbaseだけでなくクラウドプロバイダーが提供しているものもありますし、それぞれで格納できるドキュメントのサイズ等、仕様も異なっています。この辺りは、まだ単純なところですが」
...
「確かに。でも、どこから手を付ければ良いのか...」

「私が知っている範囲で、NoSQLの性能比較について、三つのリサーチペーパーが公開されています。一つは、Couchbase ServerとMongoDB、そしてDataStax Enterpriseを比較したもので、環境としてはAWSがIaaSとして使われています。@<fn>{nosql-performance-benchmark}もう一つは、DBaaSとしてのNoSQLを比較したもので、Couchbase ServerのDBaaSであるCouchbase Capellaと、MongoDBのDBaaSであるMongoDB Atlas、そしてAmazon DynamoDBとを比較しています。@<fn>{nosql-dbaas-performance-benchmark}最後は、やはりDBaaS同士を比較したもので、Couchbase CapellaとAzure Cosmos DBとを比較しています。@<fn>{nosql-dbaas-performance-benchmark-azure}後でそれぞれのURLを送っておきます」
