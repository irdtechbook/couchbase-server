...
ここでは、このベンチマークについて、引用を行いながら、全体の要約を試みます。
要約に際して、偏った解釈を交えないよう心掛けていますが、正確な内容や詳細の確認については、原本に当たっていただきますようお願いします。
...
サイズの異なる以下の３通りのクラスターで、それぞれ異なるサイズのデータを用いて、検証が実施されています。
...
検証環境として、AWS(Amazon Web Services)が利用され、クラスターを構成するサーバーとして、下記スペックのEC2インスタンスが用いられています。
 ...
 * クラスターのサイズを問わず、全てのノードで、Dataサービス、Indexサービス、Queryサービスを有効化。
...
しかしながら実際には、Couchbase Serverにおいて、主キーに相当するものは、ドキュメントキーであり、Couchbase Serverのプライマリインデックスは、主キーとは直接関係がないものです。
...
検証に用いられたクエリの実行を最適化するためには、以下のように、実際にクエリのWHERE句で用いられている@<tt>{META().id}を指定した(セカンダリー)インデックスを作成する必要があります。

//emlist{
CREATE INDEX ycsb_secondary on ycsb(META().id);
//}

性能測定結果においても、次の「ページネーション」の検証と比べた時、最も効率が良いはずであるIDを用いた検索よりも、通常のデータの値を用いた検索の方が、遥かに高い性能が出ていることが分かります。
これは、明らかに矛盾しており、それを裏付けるように、他のデータベースでは、前者が後者に勝るという納得できる結果となっています。
この違いは、後者の検証では、(原本に記載されている通り)下記のような適切な(セカンダリー)インデックスが作成されていることから来ています。
...
まず、すべてのワークロードは、データセットが使用可能なメモリに適合することを前提として実行されました。
そのため、CouchbaseのDataサービスとIndexサービスからの読み取りはすべてRAMから行われ、インメモリの速度で実行されました。
...
Couchbaseは、大規模のクラスターと大量のデータセットを用いた検証において、オペレーション全般について十分なレベルのスケーラビリティを実証しました。
