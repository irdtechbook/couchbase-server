...
@<tt>{SELECT}キーワードに続き、JSONドキュメントのプロパティ名を指定します。
各プロパティに対して、@<tt>{AS}を使用して、プロパティにエイリアス名を指定することができます。

ワイルドカード「@<tt>{*}」を使用して、すべてのプロパティを取得することもできます。

クエリの結果は、JSONオブジェクトの配列になります。
配列の要素オブジェクトの構成は、ワイルドカード「@<tt>{*}」を使用した場合とプロパティ名を指定した場合とで、異なります。

ワイルドカード「@<tt>{*}」を使用した場合、配列の要素ごとに、データベース名をキーとする辞書型オブジェクトで構成されます。この辞書型オブジェクトは、ドキュメントの各プロパティが、キーと値のペアとして含まれます。
...
クエリでプロパティ名を指定した場合、配列の要素オブジェクトに、指定されたプロパティが、キーと値のペアとして含まれます。
...
@<tt>{Database}オブジェクトを作成する時に使用したデータベースの名前をデータソース名として使うこともできます。
...
クエリ内のデータベース名、プロパティ名、エイリアス名、パラメータ名、関数名、インデックス名のような識別子には、以下のような文字を利用することができます。
...
クエリの識別子として利用可能な文字と、JSONのプロパティ名などに利用することができる文字の範囲は異なります。
...
ドキュメントのプロパティを参照するためにプロパティ式が使用されます。以下のような使い方があります。

//blankline

 * ドット構文を使用して、ネストされたプロパティを参照します。
 * 角括弧（@<tt>{[} @<tt>{]}）を使った添え字構文を使用して、配列内の項目を参照します。
 * アスタリスク(@<tt>{*})を、SELECT句の結果リストでのみ、すべてのプロパティを表すために使用できます。
 * プロパティ式の前にデータソース名またはそのエイリアス名を付けて、その起源を示します（クエリ内で一つのデータソースのみが利用されている場合は省略可能です）。
 ...
 * @<tt>{ANY/SOME}: 配列内の少なくとも1つの項目が式を満たしている場合にTRUEを返します。そうでない場合は、FALSEを返します。@<tt>{ANY}と@<tt>{SOME}は同義です。
...
上記キーワードのいずかに続き、評価される配列内の各要素、または要素オブジェクトのプロパティを指定します。
配列の要素を表現するために上記例の@<tt>{v}のように任意の変数を用いることができます。

@<tt>{IN}キーワードに続いて評価する配列型のプロパティ名を指定します。
 ...
 * @<tt>{IS MISSING}: MISSINGに等しい(プロパティが定義されていない)
 * @<tt>{IS NOT MISSING}: MISSINGに等しくない(プロパティが定義されている)
 * @<tt>{IS VALUED}: NULLでも、MISSINGでもない(プロパティが定義されており、@<tt>{null}以外の値を持つ)
 * @<tt>{IS NOT VALUED}: NULLまたは、MISSINGのいずれか(プロパティが定義されていないか、定義されていても値が@<tt>{null}である)
...
オペランドの1つがTRUEと評価された場合、TRUEを返します。それ以外の場合はFALSEを返します。
 ...
 * @<tt>{NO}: 他の照合の接頭辞として使用して、それらを無効にします（例えば、@<tt>{NOCASE}は、大文字と小文字を区別しません）。
...
上記のようなシンプルなクエリの場合は不要ですが、@<tt>{META(<データソース名>)}のように、
データソース名を引数に指定することによって、メタデータを取得する対象を識別することが可能です。
