...
Membase社は、memcachedプロジェクト@<fn>{https://memcached.org/}のリーダー等により開発されたキーバリューストアMembaseを提供していました。CouchOne社は、ドキュメント指向データベースApache CouchDBプロジェクト@<fn>{https://couchdb.apache.org/}の主要メンバーを擁し、CouchDBの商用サポートを提供していました。

2012年にリリースされたCouchbase Server 2.0により、Membaseを前身とするCouchabse Serverは、純粋なキーバリューストアから、JSONデータベース(ドキュメント指向データベース)へと変化を遂げました。

ドキュメント指向データベースであるCouchbase Serverは、JSONデータを扱うために必要な機能を提供している一方、キーバリューストアとしての側面を持ち、JSON以外のデータをバリューとして格納することも可能です。また、memcachedのように、揮発性のキャッシュとして利用するためのオプションも提供していますが、デフォルトではデータをディスクに永続化します。永続化機構において、メモリファーストアークテクチャーが採用されており、データ操作（リード・ライト）において、ミリ秒以下の応答性能を実現しています。

上に素描したCouchbase Serverのコア機能は、Dataサービスと呼ばれます。

====[column]
CouchbaseServerは、複数の種類のCouchbaseサービスからなります。本書では、一般名詞と明確に区別するため、Dataサービスのようにサービスの名称には英語表記を用いています。
...
Couchbase Server 4.0にて、N1QL(Non 1-st Normalization form Query Language)が導入され、Couchbase Serverに格納されているデータに対して、SQL準拠のクエリ言語を用いてアクセスすることができるようになりました。N1QLは、日本語で非第一正規化形クエリ言語という意味を持ち、ニッケルと発音します。
...
Couchbase公式サイト About Us@<fn>{fc3bb4bcbc203182d2d156144f1125a8}

Couchbaseブログ Couchbase Server Editions Explained@<fn>{c1e26e66ec34813926bb36298e7e7fe9}
...
デジタルトランスフォーメーション、DXについて、経済産業省資料『「DX推進指標」とそのガイダンス』@<fn>{https://www.meti.go.jp/press/2019/07/20190731003/20190731003-1.pdf}では、以下のように定義されています。
...
キャッシュ層が、アプリケーションにとってのインターフェイスとして、データの読み込み要求を受けた際に、そのデータがキャッシュの中にあるかどうかを調べ、なければデータベースからロードし、将来の別の要求のためにキャッシュの中に保存する。
この構成により、キャッシュ層における最適化（例えばデータのプリフェッチ）も可能になる。

==== ライト・スルー

キャッシュ層が、アプリケーションにとってのインターフェイスとして、データへの書き込み要求を受けた際に、データベースへの書き込みと、キャッシュの更新を同時に行う。
...
RDBにおけるいわゆるWAL/REDOログ。Redisでは、AOF (Append Only File)と呼ばれます。

=== インメモリDB

==== Oracle Coherence

Oracle Coherence@<fn>{dee7910fc6a7f7780ac7831f794eda19}では、上記のデータ永続化パターン（リード/ライト・スルー、ライト・ビハインド）を想定した実装手法が確立されています。

==== Redis

Redis@<fn>{522195171ed14f78e1f33f84a98f0de6}をMongoDBと連携する場合には、cdata社の CData Sync@<fn>{89abbd125f9fb071d8e485290dacbc26}を利用することができます。
...
以下に、以上説明したアーキテクチャーのイメージを示します(画像は、Couchbase Under the Hood: An Architectural
Overview@<fn>{9a6cce3336b02e30ca1fb60d9b70e9ad}より引用)。

//image[memory_first_arch][memory first arch]{
//}

====[column]
CouchbaseServerは、永続化要件のないデータの扱い についても、サポートしています。これは、「バケット」と呼ばれるデータ格納単位の作成時にEphemeral（ 一時的）バケットとして作成することによって実現されます（Couchbase Serverは、Memcachedプロジェクトのメンバー により開発されたMembaseをその前身 としており、Memcachedのユースケー スへの適応が意識されています）。
...
まず、先行する技術である、XMLとの比較から、JSONの特徴を解き明かしていきたいと思います。

===== XML

ユーザーが定義したタグを用いて文章構造を記述するマークアップ言語
...
「データ交換」のユースケースの中でも、特に重要だったのが、WEBブラウザー上での扱いだったといえます。WEBブラウザー上で実行可能なスクリプト言語の仕様として登場した、非同期更新技術(Ajax)は、WEBアプリケーションに、ネイティブアプリケーションと遜色のない、ユーザー体験を提供することを実現しました。この非同期更新技術のデータ交換フォーマットとして用いられたのが、当初はXMLであり、後にはJSONが、実質的に取って代わることになります。

その結果が、現在のSPA(シングルページアプリケーション)、PWA(Progressive web application)の隆盛につながっています。
そして、このような新しいUI技術（SPA, PWA）がなぜ重要かと言えば、ページビューの増加や離脱率の減少に、繋がるからに他なりません。

さらには、XMLにおいては広く浸透したとは言い難いデータ管理の目的においても、JSONは、そのデータ記述言語としての（これまでのデータ管理の主流であったRDBに遥かに勝る）柔軟性により、多くのデータベースにおいて、採用されることになりました。

==== 参考情報

Wikipedia Ajax@<fn>{1a4091951f5fb1f307e2ee166df63c1d}

Wikipedia SPA(シングルページアプリケーション)@<fn>{4cbf4c2bb4d7602315f6b0d7af6745be}

Wikipedia PWA(Progressive webapplication)@<fn>{7421ae409426b8fcd3cb845fe195a411}
...
==== Couchbaseのマルチ・ドキュメント・トランザクションは何であるか

Couchbaseのマルチ・ドキュメント・トランザクションは、あるバケットに含まれる複数ドキュメントに跨がることができます（さらには、異なるバケットに跨がることさえできます）。
...
ここで注意しなければならないのは、Couchbase(NoSQL)のマルチドキュメントトランザクションは、楽観的平行性制御に基づくものであって、ここでは、上で触れた単一ドキュメント更新時のように、悲観的ロックを選択することはできないことです。
...
NoSQLデータベースの世界では、JSONを扱うデータベースは、「ドキュメント指向データベース（ドキュメントストア）」と呼び習わされてきました。「ドキュメント」は普通名詞としてあまりに一般的すぎるため、リレーショナル・構造化データベースに対比して、「準構造化データベース」とでも呼んだ方が、技術的には、実態をよりよく表現することができるといえます（あるいは、いっそJSONデータベースと読んだ方が、実態が明確になるでしょう）。

=== 「準（半）構造化データ」という呼称について

「semi-structured data」という用語には、暗にstructured(構造)イコール、リレーショナルデータベースのテーブル形式、ということが含意されている、と解釈できます。「構造」という言葉を素直に受け止めるのであれば、これはリレーショナルデータベースに対する行き過ぎた偏向であって、テーブル形式でないデータも、紛れもなく「構造」を持っているといえます。

テーブル形式は、リレーショナルデータベースにデータを格納する際の制約である、第一正規化を強制された構造、ということができ、その意味では、「準構造化データ」というよりも、「非第一正規化形データ」とでも呼ぶ方が、実態を表しているのではないでしょうか？(ここでの疑問形は、流通している用語を変更することを訴えるためではなく、そうした地点から考えてみることを示唆する目的から使われています)

=== RDBのスーパーセットとしてのCouchbase Server

Couchbase Serverのクエリ言語には、N1QL (Non-1st-normarization-form Query Language)という名称が付けられています。N1QLでは、SQLをベースに、JSON固有の要素、配列やネスト構造に対して、構文が拡張されています。視点を変えれば、JSON固有の要素が存在しない場合、RDBで使われているものと同じSQLを使うことができるといえます。そうした面から、Couchbase ServerはRDBのスーパーセットである、ということさえ言えるかもしれません(データ形の種類について、JSONはSQL標準よりも限定されています。また、トランザクションについても、RDBよりも限定的であることは忘れてはなりません)。依然として存在する様々な違いに関わらず、このような表現をした意図としては、NoSQLという名称が、RDBへのアンチテーゼとしての意味合いを持ち、NoSQLデータベース一般がそうしたものとして目されている、という背景に対して、行き過ぎを正す意図があります。多くのNoSQLが、RDBと異なる（ビッグデータや分析等の）ユースケースで使われることを前提としているのに対して、Couchbase Serverは、インタラクティブなアプリケーションのバックエンドデータベースとして、設計・最適化されており、これはRDBのユースケース（の大部分）と共通であるといえます。



//footnote[https://memcached.org/][https://memcached.org/]

//footnote[https://couchdb.apache.org/][https://couchdb.apache.org/]

//footnote[https://www.meti.go.jp/press/2019/07/20190731003/20190731003-1.pdf][https://www.meti.go.jp/press/2019/07/20190731003/20190731003-1.pdf]

//footnote[1433ffe25ca932a2c5c0242cfc86b3e3][https://www.cdata.com/drivers/couchbase/]

//footnote[fc3bb4bcbc203182d2d156144f1125a8][https://www.couchbase.com/about]

//footnote[c1e26e66ec34813926bb36298e7e7fe9][https://blog.couchbase.com/couchbase-server-editions-explained-open-source-community-edition-and-enterprise-edition/]
