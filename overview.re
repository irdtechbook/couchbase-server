...
Couchbase Serverは、JSONデータを扱うために必要な機能を提供する一方、キーバリューストアとしての側面を残しており、JSON以外のデータをバリューとして格納することも可能です。また、memcachedのように、揮発性のキャッシュとして利用するためのオプションも提供しています(デフォルトではデータをディスクに永続化します)。永続化機構において、メモリファーストアークテクチャーが採用されており、読み込みおよび書き込みのデータ操作において、ミリ秒以下の応答性能を実現しています。
...
=== イベント処理
Couchbase Server 5.5にて、イベント処理機能(Eventingサービス)が導入されました。ユーザーは、「イベント-コンディション-アクション」モデルに基づいてトリガーされるJavaScriptを用いたユーザー定義関数を利用できるようになりました。ユーザーは、Couchbase Server内のデータ更新・削除(イベント)に対して、条件(コンディション)と、それに対するユーザー定義関数(アクション)を定義します。ユーザ定義関数では、Couchbase Serverのデータに対する操作のほか、REST APIコールによる外部サービス呼び出しを実行することが可能です。
...
インメモリについては、オープンソースのNoSQLとしては、Redisが代表的と言えます。一般には、インメモリデータベースは、（単体でアプリケーションのキャッシュとして用いるのでなければ）永続化層を担う他のデータベースと組み合わせて利用することによって、高い応答性能という特性を持つデータプラットフォームを実現することができます。
...
ここでは、Couchbase Serverのメモリーファーストアーキテクチャーについて理解するため、まずインメモリDBという技術一般について検討します。
また、その具体的な実現としてOracl CoherenceとRedisを取り上げます。これらは、いずれもインメモリ分散KVS(キーバリューストア)というカテゴリーに入ります。また、どちらもコミュニティエディションの存在するOSSベースのソフトウェアです。

インメモリDBは、その名の通りデータをメモリに格納します。アプリケーション(データベース利用者)からみたインメモリDBの性能面の特徴としては、永続化装置の性能がボトルネックにならないデータベースであると言えます(もっとも、インメモリDBはそもそも永続化の機構を担っていないため、これ自体はさほど意味のない表現ですが)。このデータベースとしての特徴を、永続化を担う他のシステムと組み合わせることによって、従来のデータベースとは異なるレベルの高い応答性能を実現するアーキテクチャーを構築することができます。
...
データ永続化の課題をキャッシュ層における障害発生時のデータ復旧のためのバックアップという観点で捉えることもでき、以下のようなアプローチが存在します(本節の全体の論旨の上では傍系的な話題となりますが、網羅的にテーマを概観するため整理します)。
...
冒頭にあげた、２つのインメモリDBについて、永続化手法との関係を整理します。

 * @<strong>{Oracle Coherence}@<fn>{dee7910fc6a7f7780ac7831f794eda19}では、上記のデータ永続化パターン（リード/ライトスルー、ライトビハインド）を想定した実装手法が確立されています。永続化層としてはOracle Databaseが用いられます。
...
これまで見たように、インメモリDBを利用する場合、通常データの永続化は、ユーザーの選択した方法によるインテグレーションにより実現されます。Couchbase Serverのユニークなところは、メモリ「ファースト」アーキテクチャーの採用にあります。
ユーザーは、メモリ上のデータへのアクセス（ディスク利用による性能制約からの開放）による応答性能を享受しながら、データは、あたかもごく一般的なデータベースのように、ディスク上に保存されている状態が、シングルプラットフォームで実現されています(ここで、「シングルプラットフォーム」という表現は、このようなキャッシュと永続化の両方を満たすための典型的なアーキテクチャーとして、キャッシュレイヤーと永続化レイヤーを２つの異なるテクノロジーによって実現するケースとの対照として用いられています)。
...
そうした背景から、メモリのみを利用して永続化要件のないデータを扱うこともサポートされています。
...
ここでは、なぜJSON(データベース)が、現在のアプリケーション開発において、重要なのかを見ていきます。
...
ここでは、データベース技術の重要なトピックであるトランザクションについて、データベース技術者がNoSQL(ドキュメント指向データベース)に触れる際に有益と思われる論点を整理しながら、歴史的な展望と併せて、見ていきます。
...
Couchbase Serverについてのそのような例として、NECのInfoFrame Relational Store(IRS)@<fn>{66fe5150939f305857c0310ca092a9bc}があります。
...
後者については、一概に結論付けられるようなものではなく、アークテクチャー的なアドバンテージなど諸々を総合的に考慮した上で、判断が必要になると思われますが、よく知られた、少なくない数のリレーショナルデータベースで、デフォルトの分離レベルとしてリードコミッテッドが採用されているのも事実です。
...
この記事では、RDBMSとNoSQL(ドキュメント指向データベース)、そしてNewSQLのトランザクションを比較した上で、Couchbase Serverにおけるマルチドキュメント分散ACIDトランザクションがどのように実装されているかを解説しています。
